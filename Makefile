# This Makefile is for the Text::Markdown extension to perl.
#
...
#     ABSTRACT => q[Convert Markdown syntax to (X)HTML]
#     AUTHOR => q[Tomas Doran <bobtfish@bobtfish.net>]
#     DIR => []
#     DISTNAME => q[Text-Markdown]
#     NAME => q[Text::Markdown]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::Exception=>q[0], FindBin=>q[0], ExtUtils::MakeMaker=>q[6.42], Encode=>q[0], Test::More=>q[0.42], Text::Balanced=>q[0], Digest::MD5=>q[0], List::MoreUtils=>q[0], File::Slurp=>q[0] }
#     VERSION => q[1.0.22]
...
TO_INST_PM = lib/Text/Markdown.pm \
	lib/Text/MultiMarkdown.pm

PM_TO_BLIB = lib/Text/Markdown.pm \
	blib/lib/Text/Markdown.pm \
	lib/Text/MultiMarkdown.pm \
...
DISTNAME = Text-Markdown
DISTVNAME = Text-Markdown-1.0.22
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Text/Markdown.pm $(INST_MAN3DIR)/Text::Markdown.$(MAN3EXT) \
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="1,0,22,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Convert Markdown syntax to (X)HTML</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Tomas Doran &lt;bobtfish@bobtfish.net&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Text/Markdown.pm blib/lib/Text/Markdown.pm \
